...
fastify.post('/auth/signup', async (req, reply) => {
  ...
  if (tErr) return reply.status(400).send({ error: 'Invalid tenant' });
  ...
  const { data: profile, error: pErr } = await supabase
    .from('users').insert({ id: authData.user.id, tenant_id: tenant.id, email, name, role }).single();
  if (pErr) {
...
fastify.get('/auth/me', async (req, reply) => reply.send(req.user));
...
fastify.post('/dogs/:id/friends/:friendId', async (req, reply) => {
  ...
  const { data, error } = await supabase.from('dog_friendships').insert(payload).single();
  if (error) return reply.status(400).send({ error });
  reply.send(data);
});
